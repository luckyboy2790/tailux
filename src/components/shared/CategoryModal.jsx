...
export function CategoryModal({ type, row, isOpen, close }) {
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/category/save`, {
        method: "POST",
        body: formData,
        headers: {
            ...
            <DialogPanel className="dark:bg-dark-700 relative flex w-full max-w-lg origin-top flex-col overflow-hidden rounded-lg bg-white transition-all duration-300">
              <div className="dark:bg-dark-800 flex items-center justify-between rounded-t-lg bg-gray-200 px-4 py-3 sm:px-5">
                ...
                >
                  {t(
                    type === "add"
                      ? "nav.category.add_category"
                      : "nav.category.edit_category",
                  )}
                </DialogTitle>
                <Button
                  onClick={close}
              ...
              <div className="flex flex-col overflow-y-auto px-4 py-4 sm:px-5">
                <div className="mt-4 space-y-5">
                  <Input
                    placeholder={t("nav.category.category_name")}
