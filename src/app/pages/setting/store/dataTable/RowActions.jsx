...
export function RowActions({ row, table }) {
  ...
  const confirmMessages = {
    pending: {
      description: t("nav.store.confirmDelete.pending.description"),
    },
    success: {
      title: t("nav.store.confirmDelete.success.title"),
    },
  };
  ...
  const handleDeleteRows = async () => {
    ...
    const response = await fetch(
      `${API_URL}/api/store/delete/${row.original?.id}`,
      {
        method: "POST",
      },
    );
    ...
    if (!response.ok) {
      toast.error(t("nav.store.confirmDelete.failed.title"));
    ...
    toast.success(t("nav.store.confirmDelete.success.title"));
