    ...
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(
          `${API_URL}/api/report/product?${queryString}`,
        );
                    ...
                    {isLoading ? (
                      ...
                      <>
                        ...
                        {table.getCoreRowModel().rows.length > 0 && (
                          <Tr>
                            <Td colSpan={3}>{t("nav.detail.sub_total")}</Td>
                            <Td>
                              $
                              {table
                                .getCoreRowModel()
                                .rows.reduce(
                                  (acc, row) =>
                                    acc + row.original.purchased_quantity,
                                  0,
                                )
                                .toLocaleString()}
                            </Td>
                            <Td>
                              ...
                              {table
                                ...
                                .rows.reduce(
                                  (acc, row) =>
                                    acc + Number(row.original.sold_quantity),
                                  0,
                                )
                                .toLocaleString()}
                            </Td>
                            <Td>
                              ...
                              {table
                                ...
                                .rows.reduce(
                                  (acc, row) =>
                                    acc + Number(row.original.purchased_amount),
                                  0,
                                )
                                .toLocaleString()}
                            </Td>
                            <Td>
                              ...
                              {table
                                ...
                                .rows.reduce(
                                  (acc, row) =>
                                    acc + Number(row.original.sold_amount),
                                  0,
                                )
                                .toLocaleString()}
                            </Td>
                              ...
                              {(() => {
                                const total = table
                                  ...
                                  .rows.reduce(
                                    (acc, row) =>
                                      acc +
                                      Number(row.original.sold_amount) -
                                      Number(row.original.purchased_amount),
                                    0,
                                  );

                                return `${total < 0 ? "-" : ""}$${Math.abs(
                                  total,
                                ).toLocaleString()}`;
                              })()}
                            </Td>
